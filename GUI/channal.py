...
class Channal:
    ...
    def generate_waveform(self):
        ...
        samples = generateSamples(self.waveform_type, 1000, self.ampInput.value, self.arbitrary_waveform,
                                  self.DSInput.value, self.phaseInput.value / 360, offset=self.offsetInput.value,
                                  timeRange=tr, clamp=[-10, 10])
        ...
        self.guide_lines[1].setData([-tr, tr * 2], [self.offsetInput.value + self.ampInput.value,
                                                    self.offsetInput.value + self.ampInput.value])
        self.guide_lines[2].setData([-tr, tr * 2], [self.offsetInput.value - self.ampInput.value,
                                                    self.offsetInput.value - self.ampInput.value])
        self.guide_lines[1].setVisible(self.waveform_type != "dc")
        self.guide_lines[2].setVisible(self.waveform_type != "dc")

        if self.running:
            self.conn.sendWave(self.chan_num, freq=self.freqInput.value, wave_type=self.waveform_type,
                               amplitude=self.ampInput.value, offset=self.offsetInput.value, arbitrary_waveform=None,
                               duty=self.DSInput.value, phase=self.phaseInput.value / 360)
        else:
